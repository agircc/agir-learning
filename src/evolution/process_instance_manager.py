...
class ProcessManager:
    ...
    @staticmethod
    def _get_initial_node(db: Session, process_id: int) -> Optional[State]:
        """
        ...
        Returns:
            Optional[State]: Initial node if found, None otherwise
        """
        try:
            # Get all nodes in the process
            all_nodes = db.query(State).filter(State.scenario_id == process_id).all()
            if not all_nodes:
                logger.error(f"No nodes found for process: {process_id}")
                return None
            
            # Get all 'to' nodes in transitions
            to_nodes = db.query(StateTransition.to_state_id).filter(
    ...
    def advance_process(instance_id: int, next_node_name: Optional[str] = None) -> Optional[int]:
        ...
        try:
            ...
            instance = db.query(Episode).filter(Episode.id == instance_id).first()
            if not instance:
                logger.error(f"Process instance not found: {instance_id}")
                return None
            
            # Get current node
            current_node_id = instance.current_state_id
            ...
            if next_node_name:
                ...
                next_node = db.query(State).filter(
                    State.scenario_id == instance.scenario_id,
                    State.name == next_node_name
                ).first()
            ...
            if not next_node_id:
                ...
                transition = db.query(StateTransition).filter(
                    StateTransition.scenario_id == instance.scenario_id,
                    StateTransition.from_state_id == current_node_id
                ).first()
                
                if transition:
                    next_node_id = transition.to_state_id
            ...
            instance.current_state_id = next_node_id
            db.commit()
    ...
    def complete_process(instance_id: int, success: bool = True) -> bool:
        ...
        try:
            ...
            instance.status = EpisodeStatus.COMPLETED if success else EpisodeStatus.FAILED
            db.commit()
    ...
    def get_process(process_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            process = db.query(Scenario).filter(Scenario.id == process_id).first()
            ...
            process_instance = db.query(Episode).filter(
                Episode.scenario_id == process_id
